...
// Self-hosted brand icon (public/icons/<id>.png — generated by scripts/gen-icons.ts
// for every provider, so there's no letter fallback and no CDN dependency). If a
// file is ever missing (a provider added before icons regenerate), the <img> is
// removed and the blank tile shows — never a letter.
function logoTile(p) {
  return `<span class="logo"><img src="/icons/${esc(p.id)}.png" alt="" loading="lazy" onerror="this.remove()"></span>`;
}
