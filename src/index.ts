...
export default {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (request.method === "GET" && (url.pathname === "/status" || url.pathname.startsWith("/status/") || url.pathname === "/compare" || url.pathname.startsWith("/compare/") || url.pathname === "/sitemap.xml" || url.pathname === "/llms.txt" || ["/privacy", "/terms", "/subprocessors", "/security", "/about", "/support", "/mac", "/alerts"].includes(url.pathname))) {
      const cache = caches.default;
